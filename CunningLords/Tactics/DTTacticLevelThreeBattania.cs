...
{
    class DTTacticLevelThreeBattania : TacticComponent
    {
		...
		public DTTacticLevelThreeBattania(Team team) : base(team)
		{
		...
		protected override void TickOccasionally()
		{
			...
			{
				//InformationManager.DisplayMessage(new InformationMessage("Level 3 Battania"));
				...
				List<Formation> enemyCavalry = Utils.PlayerFormationsOfType(FormationClass.Cavalry, this._mainInfantry);
				Formation infantryTarget = null;
				if (enemyCavalry != null && enemyCavalry.Count >= 0)
				{
					infantryTarget = enemyCavalry.First();
				}
				...
				DecisionIsFormationNotNull decisionEnemyCavalryNotNull = new DecisionIsFormationNotNull(infantryTarget, infantryCharge, decisionInfantryCycleCharge);

				DecisionClosestEnemyCloserThan decisionCloserThan = new DecisionClosestEnemyCloserThan(this._mainInfantry, decisionEnemyCavalryNotNull, infantryAdvance, 30f);
				...
				ActionSparseSkirmish archersSparseSkirmish = new ActionSparseSkirmish(this._archers);
				...
				DecisionIsFormationNotNull decisionEnemyCavalryNotNullII = new DecisionIsFormationNotNull(infantryTarget, archersScreenedSkirmish, archersSparseSkirmish);

				DecisionIsAttacker decisionIsArchersAttacker = new DecisionIsAttacker(this._archers, decisionEnemyCavalryNotNullII, archersHoldLine);
